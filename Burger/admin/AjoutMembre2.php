    ...
    if(!empty($_POST))
    {
        ...
        */
        ...
        if($isSuccess)
        {
            ...
            //On insert
            $statement = $db->prepare("INSERT INTO `utilisateur` (`iduser`, `pseudo`, `nom`, `prenom`, `email`, `password_hash`, `type_idType`) VALUES (NULL, '$pseudo', '$nom', '$prenom', '$email', '$empreinte', 2)");
...
-->
                    ...
                    <label for="users">Vous &ecirc;tes </label>
                            <select class="form-control" id="type_idType" name="type_idType">
                                <?php
                                    $db = Database::connect();
                                    foreach($db->query('SELECT * FROM type_users where idtype = 2') as $row)
                                    {
