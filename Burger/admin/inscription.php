...
header('Content-Type:text/html; charset=iso-8859-1');
    ...
    if(!empty($_POST))
    {
        ...
        */
        ...
        //Si on essaie d'utiliser un même pseudo, ça ne marchera pas
...
//        {
//            $db=Database::connect();
//            $statement = $db->prepare("SELECT COUNT(*) AS nbr FROM `utilisateur` WHERE pseudo = '$pseudo'");
//            $statement->execute();
...
//            }
            ...
            /*
                ...
                0 : le pseudo n'existe pas
                1 : le pseudo existe
            */
//            $pseudo_existe = ($statement->fetchColumn()==0)?1:0;
...
//mysql_select_db("webburger");
//            
//            $statement = mysql_query("SELECT `pseudo` FROM `utilisateur` WHERE `pseudo` = '$pseudo'");
//            
//            $verdict = mysql_fetch_array($statement, MYSQL_ASSOC);
//            //On execute
//            //$statement->execute(array($pseudo));
//            
//            //mysql_fetch_assoc($statement
//            
//            if( $pseudo == $verdict ['pseudo'])
//            {
////                 $erreur['pseudo'] = 'Cet email est d&eacute;j&agrave; pris pour un autre compte !';
//                
//                echo("Ce pseudo existe d&eacute;j&agrave;. Veuillez changez !");
//                exit;
//            }
//            
...
//            else
//            {
//                $erreur['email'] = 'Cet email est d&eacute;j&agrave; pris pour un autre compte !';
//                
//            }
...
//        {
//            $db = Database::connect();
//            $statement = $db->prepare("SELECT `pseudo` FROM `utilisateur` WHERE `pseudo` = '$pseudo'");
//            
//            //On execute
//            $statement->execute(array($pseudo));
//            Database::disconnect();
...
//        }
        ...
        if($isSuccess)
        {
            ...
            //On insert
            $statement = $db->prepare("INSERT INTO `utilisateur` (`iduser`, `pseudo`, `nom`, `prenom`, `email`, `password_hash`, `type_idType`) VALUES (NULL, '$pseudo', '$nom', '$prenom', '$email', '$empreinte', 3)");
...
<html>
    ...
    <body>
        ...
        <div class="container admin">
            <div class="row">
                <h1><strong><center>Inscription</center></strong></h1>
                <br />
...
-->
                    ...
                    <label for="users">Vous &ecirc;tes </label>
                            <select class="form-control" id="type_idType" name="type_idType">
                                <?php
                                    $db = Database::connect();
                                    foreach($db->query('SELECT * FROM type_users where idtype = 3') as $row)
                                    {
                    ...
                    <div class="form-actions">
                            ...
                            <a class="btn btn-default" href="../accueil.php"><span class="glyphicon glyphicon-arrow-left"></span> Retourner &agrave; l'accueil</a>
                    </div>
                    <br />
